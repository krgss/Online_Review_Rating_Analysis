...
void svm_learn_classification(DOC **docs, double *yclass, long int
							  ...
							  /* yclass:       Training labels (y-part, zero if test example for
							  transduction) */
						  ...
						  /* model:       Returns learning result (assumed empty before called) */
{
	...
	temporary documents */
	model->primal = (int*)my_malloc(sizeof(int) * model->sv_num + 1);
	for(i=1; i<model->sv_num; i++) { 
		model->primal[i] = 1;
							 ...
							 /* retrain: selects training mode (1=regular / 2=holdout) */
{
	...
	for(; retrain && (!terminate); iteration++) 
	{
		if(kernel_cache)
			kernel_cache->time=iteration;  /* for lru cache */
		if(verbosity>=2) {
			if ( iteration % 100 == 0 ) {printf("Iteration %ld: ",iteration); fflush(stdout);}
		} else if(verbosity==1) {
			if ( iteration % 100 == 0 ){ printf("."); fflush(stdout);}
		}
		...
		for( jj=0; (j=working2dnum[jj])>=0; jj++) { /* clear working set */
			if((chosen[j]>=(learn_parm->svm_maxqpsize/ minl(learn_parm->svm_maxqpsize,
		...
		if((!retrain) && (inactivenum>0) && ((!learn_parm->skip_final_opt_check) 
			|| (kernel_parm->kernel_type == LINEAR))) 
		{ 
			if(((verbosity>=1) && (kernel_parm->kernel_type != LINEAR)) || (verbosity>=2)) 
			{
