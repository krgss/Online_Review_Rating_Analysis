...
char docfile_classify[200];
char modelfile_classify[200];
char predictionsfile_classify[200];
...
{
	int nAcc = 0;
	for ( int m=0; m<totdoc; m++ ) {
		double dist = classify_example(model, docs[m]);
		if ( dist > 0 && target[m] > 0 ) nAcc ++;
		if ( dist < 0 && target[m] < 0 )	nAcc ++;
	}
	return nAcc;
}
...
int classify(int argc, char* argv[])
{
  ...
  read_input_parameters(argc,argv,docfile_classify,modelfile_classify,predictionsfile_classify,
			&verbosity,&pred_format);

  nol_ll(docfile_classify,&max_docs,&max_words_doc,&lld); /* scan size of input file */
  max_words_doc += 2;
  lld += 2;
  ...
  model = read_model(modelfile_classify);
  ...
  if ((docfl = fopen (docfile_classify, "r")) == NULL)
  { perror (docfile_classify); exit (1); }
  if ((predfl = fopen (predictionsfile_classify, "w")) == NULL)
  { perror (predictionsfile_classify); exit (1); }
			   ...
			   long int *verbosity, long int *pred_format)
{
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      { 
      case 'h': print_help_classify(); exit(0);
