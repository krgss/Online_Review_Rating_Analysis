...
char testfile[200];           /* file with training examples */
char modelfile[200];         /* file for resulting classifier */
char restartfile[200];       /* file with initial alphas */

void   read_input_parameters(int, char **, char *, char *, char *, char *, long *, 
							 LEARN_PARM *, KERNEL_PARM *);
void   wait_any_key();
void   print_help();

int learn(DOC **docs, double *target, long totwords, long totdoc, double *alpha_in,
		  KERNEL_CACHE *kernel_cache, LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm,
		  MODEL *model)
{
...
void getData(DOC ***doc, long totdoc, DOC ***trDoc, DOC ***tstDoc, 
			 double **target, double **trTarget, double **tstTarget, 
			 long &ntotTrDoc, long &ntotTstDoc, int foldIx)
{
	int nunit = totdoc / 5;
	ntotTrDoc = ntotTstDoc = 0;

	for ( int i=0; i<totdoc; i++ )
	{
		bool btrain = true;
		if ( foldIx < 5 && (i>=(foldIx-1)*nunit) && (i<foldIx*nunit) ) btrain = false;
		else if ( foldIx == 5 && (i >= (foldIx-1) * nunit) ) btrain = false;
		
		if ( btrain ) {
			(*trDoc)[ntotTrDoc] = (*doc)[i];
			(*trTarget)[ntotTrDoc] = (*target)[i];
			ntotTrDoc ++;
		} else {
			(*tstDoc)[ntotTstDoc] = (*doc)[i];
			(*tstTarget)[ntotTstDoc] = (*target)[i];
			ntotTstDoc ++;
		}
	}
}
...
{  
	...
	string str(buff);
	if ( str.find("classify") != str.npos ) { // classify examples
		return classify(argc, argv);
	} 

	// select the best C parameter by cross-valiation on the training data
	...
	read_input_parameters(argc, argv, docfile, modelfile, testfile, restartfile,&verbosity,
		&learn_parm, &kernel_parm);
	...
	// the default/input C parameter
	double dBestC = learn_parm.svm_c;

	/* do 5 fold cross-validation to select good C. */
	if ( str.find("inner-cv") != str.npos ) {
		int nMaxUnit = totdoc / 5 + 5;

		DOC **trDoc = (DOC **)my_malloc(sizeof(DOC *) * (totdoc - totdoc / 5) );
		DOC **tstDoc = (DOC **)my_malloc(sizeof(DOC *) * nMaxUnit);
		double *trTarget = (double*)my_malloc(sizeof(double) * (totdoc - totdoc / 5));
		double *tstTarget = (double *)my_malloc(sizeof(double) * nMaxUnit);

		double dBestAccuracy = 0;
		double vecC[37] = {0.001, 0.01, 0.03, 0.09, 0.11, 0.13, 0.17, 0.19//};
			,0.21, 0.23};//, 0.25, 0.27, 0.29, 0.31};//, 0.33, 0.35, 0.37, 0.41, 0.43, 0.45, 0.47};
		for ( int k=0; k<10; k++ )
		{
			printf("\n\n$$$ Learning with C: %.4f $$$ \n\n", vecC[k]);

			learn_parm.svm_c = vecC[k];
			double dAvgAccuracy = 0;
			for ( int i=1; i<=5; i++ )
			{
				// get training & test data
				long ntotTrDoc, ntotTstDoc;
				getData(&docs, totdoc, &trDoc, &tstDoc, &target, &trTarget, &tstTarget, 
					ntotTrDoc, ntotTstDoc, i);
				
				MODEL *trModel = (MODEL *)my_malloc(sizeof(MODEL));

				learn(trDoc, trTarget, totwords, ntotTrDoc, alpha_in, kernel_cache, &learn_parm,
					&kernel_parm, trModel);

				//write_model(modelfile, trModel);

				// test on testing data
				int nAcc = classify(trModel, tstDoc, tstTarget, ntotTstDoc);	

				dAvgAccuracy += (nAcc * 100.0) / (ntotTstDoc * 5.0);

				free_model(trModel, 0);
				printf("\n\n");
			}
			printf("@@@ Avg Accuracy: %.4f\n\n", dAvgAccuracy);

			if ( dAvgAccuracy > dBestAccuracy )	{
				dBestC = learn_parm.svm_c;
	...
	// learn the model over the whole training set.
	learn_parm.svm_c = dBestC;
	printf("\n\n############ Best C: %.4f ########### \n", dBestC);
	learn(docs, target, totwords, totdoc, alpha_in, kernel_cache, &learn_parm, &kernel_parm, model);

	//char tstfile[512] = "reuters(first8500)_test_tfidf5000_cv5(4).txt";
	DOC **tstDocs;
	double *tstTarget;
	read_documents(testfile, &tstDocs, &tstTarget, &totwords, &totdoc);
	int nAccNum = classify(model, tstDocs, tstTarget, totdoc);
	printf("\n\nFinal Test Accuracy: %.4f\n\n", (nAccNum * 100.0) / totdoc);
...
void read_input_parameters(int argc,char *argv[],char *docfile,char *modelfile,
						   char *testfile, char *restartfile,long *verbosity,
