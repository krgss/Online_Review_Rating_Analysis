...
void write_model(char *modelfile, MODEL *model)
{
	...
	char buff[512];
	sprintf(buff, "%s_primal", modelfile);
...
MODEL *read_model(char *modelfile)
{
	...
	model->primal = (int *)my_malloc(sizeof(int) * model->sv_num);
	model->index = NULL;
	...
	char buff[512];
	sprintf(buff, "%s_primal", modelfile);
	FILE *fileptr = fopen(buff, "r");
	for (i=1; i<model->sv_num; i++ )
		fscanf(fileptr, "%d", &model->primal[i]);
	fclose(fileptr);
...
void free_model(MODEL *model, int deep)
{
	...
	if(model->primal) free(model->primal);
	free(model);
}
					...
					long int *totwords, long int *totdoc)
{
	...
	if(verbosity>=1) { printf("Reading examples into memory..."); fflush(stdout); }
	...
	while( (!feof(docfl)) && fgets(line,(int)ll,docfl)) 
	{
		...
		//if( verbosity>=1 ) 
		//{
		//	if((dnum % 100) == 0) { printf("%ld..",dnum); fflush(stdout); }
		//}
	} 
...
// parse a document (labels and feature vector)
				   ...
				   char **comment, bool bModel)
{
		...
		(numread > 0) && (wpos<max_words_doc)) 
	{
		...
		if(sscanf(featurepair,"qid:%ld%s",&wnum,junk)==1) {
			...
			// insert a feature vector (not require the features are in increasing order)
